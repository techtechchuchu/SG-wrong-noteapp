...
def show_banner():
    ...
    banner_html = """
<div style="margin-top:16px; margin-bottom:24px; padding:22px 24px; border:1px solid #ead7d2; border-left:6px solid #9f2d20; border-radius:14px; background:linear-gradient(135deg, #fffaf8 0%, #fff5f2 100%); color:#2f2f2f; line-height:1.8; box-shadow:0 6px 20px rgba(120, 60, 45, 0.08);">
  <div style="font-size:22px; font-weight:800; color:#8f2a1f; margin-bottom:12px; letter-spacing:-0.4px;">
    시스템 점검 및 계정 복구 안내
  </div>

  <div style="font-size:16px;">
    현재 오답노트 앱의 안정적인 운영을 위해 시스템 점검과 데이터 백업 기능 개선을 진행하고 있습니다.
    기존 계정 백업 과정에서 일부 데이터가 초기화되어, 학생 계정을 순차적으로 복구하고 있습니다.
    <br><br>

    <div style="padding:13px 15px; border-radius:10px; background-color:#ffffff; border:1px solid #eadfdb;">
      <b>임시 비밀번호</b>
      <span style="margin-left:8px; color:#8f2a1f; font-size:18px; font-weight:800;">sg2026</span>
    </div>

    <br>
    기존에 사용하던 학생 이름과 임시 비밀번호로 로그인해주시기 바랍니다.
    로그인 후, <b>주말에 작성했던 오답번호를 기존 작성 개수만큼 다시 입력</b>해주세요.
    <br><br>

    서비스 이용에 불편을 드려 진심으로 죄송합니다.
    보다 안정적이고 편리한 오답노트 앱으로 개선할 수 있도록 최선을 다하겠습니다.
  </div>

  <div style="margin-top:16px; text-align:right; font-size:15px; font-weight:700; color:#6b514b;">
    SG 고등관 조교 드림
  </div>
</div>
"""
        ...
        for _, row in df.iterrows():
            ...
            if exists:
                cur.execute(
                    """
                    UPDATE users
                    SET password_hash = ?, grade = ?
                    WHERE username = ?
                    """,
                    (hash_pw(password), grade, username)
                )
                updated_count += 1
            else:
                cur.execute(
                    """
                    INSERT INTO users
                    (username, password_hash, grade, created_at)
                    VALUES (?, ?, ?, ?)
                    """,
                    (
                        username,
